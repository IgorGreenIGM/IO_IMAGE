...
/**
 * @brief method for downscale an input buffer
 * 
 * @param rgb_in input rgb buffer
 * @param rgb_len input rgb buffer size
 * @param width_in rgb buffer width
 * @param height_in rgb buffer height
 * @param scale times to downscale the image(width_out = width_in/scale, height_out = height_in/scale)
 * @param width_out a reference to output width of the buffer
 * @param width_out a reference to output height of the buffer
 * 
 * @return uint8_t* downscaled buffer
 */
uint8_t *PixelsManager::downscale_buffer(const uint8_t *rgb_in, int rgb_len, int width_in, int height_in, float scale, int &width_out, int &height_out)
{
    width_out = width_in/scale;
    height_out = height_in/scale;

    std::cout << "width : " << width_out <<  " height : " << height_out << std::endl;

    if (width_out <= 1 or width_in <= 1)
        throw std::runtime_error("Error, the input scale is too big !");

    uint8_t *rgb_out = new uint8_t[width_out * height_out * 3];
    if (!rgb_out)
        throw std::bad_alloc();
    ...
    while (i < height_in)
    {
        ...
        while (j < width_in)
        {
            rgb_out[k] = rgb_in[((i * width_in) + j) * 3];
            rgb_out[k + 1] = rgb_in[(((i * width_in) + j) * 3) + 1];
            rgb_out[k + 2] = rgb_in[(((i * width_in) + j) * 3) + 2];
            
            j += std::ceil(scale); // skipping the next columns according to the scale
            k += 3; 
        }

        i += std::ceil(scale); // // skipping the next lines according to the scale
    }
